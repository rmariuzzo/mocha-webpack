...
const Watching = require("webpack/lib/Watching.js");
...
export default function createWatchCompiler(
  ...
  const createWatcher = () => Watching(compiler, watchOptions, noop);
  let watchCompiler: {
  ...
  return {
    watch() {
      if (watchCompiler === null) {
        ...
        const times = (compiler as any).watchFileSystem.watcher.getTimes();
        ...
        if (Object.keys(times).length > 0) {
          const timesMap = new Map(
            Object.keys(times).map((key) => [key, times[key]])
          );
