...
const replaceEol = (message: string) => message.replace(/\r?\n/g, "\n");
...
// strip at ... ...:x:y
const stripStackTrace = (message: string) =>
...
const cleanUpUnwantedEol = (message: string) =>
  message.replace(/\s*\n\s*$/, "");

// indent all lines by 2 spaces
const indent = (message: string) =>
  message
    .split("\n")
    .map((l) => `  ${l}`)
    .join("\n");
