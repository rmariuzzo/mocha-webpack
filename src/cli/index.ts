...
const cliOptions = parseArgv(process.argv.slice(2), true) as Record<
  string,
  string
>;
const configOptions = parseConfig(cliOptions.opts) as Record<string, any>;
...
(options.require as string[]).forEach((mod) => {
  require(resolve(mod)); // eslint-disable-line global-require, import/no-dynamic-require
});

options.include = (options.include as string[]).map(resolve);
...
(options.include as string[]).forEach((f) => mochaWebpack.addInclude(f));
...
(options.files as string[]).forEach((f) =>
