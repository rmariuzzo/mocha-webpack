...
function sortExtensions(ext1: string, ext2: string) {
  if (ext1 === ".js") {
    return -1;
  }
  if (ext2 === ".js") {
...
function findConfigFile(dirPath: string, baseName: string) {
...
function getConfigExtension(configPath: string) {
...
function registerCompiler(moduleDescriptor: any) {
  if (!moduleDescriptor) {
    return;
  }
...
export default function requireWebpackConfig(
  ...
  if (fileExists(configPath)) {
    ...
    if (configPathPrecompiled != null) {
      ...
      const configExtensionPrecompiled = getConfigExtension(
        configPathPrecompiled
      );
  ...
  config = (config as { default: Configuration }).default || config;

  if (typeof config === "function") {
    config = (config as Function)(env);
  }
