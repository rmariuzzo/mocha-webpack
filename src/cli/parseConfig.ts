...
function handleMissingConfig(config: string) {
  if (config) {
...
const createStripSurroundingChar = (c: string) => (s: string) => {
  if (
    s.indexOf(c) === 0 &&
...
const removeSurroundingQuotes = (str: string) => {
...
export default function parseConfig(explicitConfig: string) {
