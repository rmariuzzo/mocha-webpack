...
const options: Record<CliOption, Options> = {
  "async-only": {
    alias: "A",
    type: "boolean",
...
type CliOptions = typeof options;
...
const parametersWithMultipleArgs = paramList(
  _.pickBy(_.mapValues(options, (v) => !!v.requiresArg && v.array === true))
...
export default function parseArgv(argv: string[], ignoreDefaults = false) {
  ...
  const reporterOptions: Record<string, string | boolean> = {};

  if (validOptions.reporterOptions) {
    (validOptions.reporterOptions as string).split(",").forEach((opt) => {
      const L = opt.split("=");
  ...
  if (ignoreDefaults) {
    ...
    const usedAliases = _.flatten(
      _.filter(groupedAliases, (aliases) =>
        _.some(aliases, (alias) => providedKeys.indexOf(alias as string) !== -1)
      )
    );

    if ((parsedArgs as any)._.length) {
      usedAliases.push("files");
    }

    return _.pick(validOptions, usedAliases as any);
  }
