...
export default class TestRunner extends EventEmitter {
  ...
  prepareMocha(webpackConfig: Configuration, stats: Stats): Mocha {
    const mocha: Mocha = configureMocha(this.options);
    const outputPath = webpackConfig.output?.path;
    const buildStats: BuildStats = getBuildStats(stats, outputPath);

    (global as any).__webpackManifest__ = buildStats.affectedModules; // eslint-disable-line
  ...
  async watch(): Promise<void> {
    ...
    const fileDeletedOrAdded = (file: string, deleted: boolean) => {
